...
struct TUIRenderingTests {
    ...
    @Test
    func keyEventNormalization_csiCommandShiftArrows() throws {
        let parser = ProcessTerminal()
        let events = parser.parseForTests("\u{001B}[1;10D\u{001B}[1;10C\u{001B}[1;10A\u{001B}[1;10B")

        #expect(events.contains(where: {
            if case let .key(.arrowLeft, modifiers) = $0 {
                return modifiers == [.shift, .command]
            }
            return false
        }))
        #expect(events.contains(where: {
            if case let .key(.arrowRight, modifiers) = $0 {
                return modifiers == [.shift, .command]
            }
            return false
        }))
        #expect(events.contains(where: {
            if case let .key(.arrowUp, modifiers) = $0 {
                return modifiers == [.shift, .command]
            }
            return false
        }))
        #expect(events.contains(where: {
            if case let .key(.arrowDown, modifiers) = $0 {
                return modifiers == [.shift, .command]
            }
            return false
        }))
