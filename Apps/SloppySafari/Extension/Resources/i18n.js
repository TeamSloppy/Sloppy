(function installSloppyI18n(root) {
  const dictionaries = {
    en: {
      ...
      summarizePagePrompt: "Summarize this page. Focus on the main points and any actionable details.\n\nUse the Safari extension browser tool browser.dom_snapshot to read the active page content.\nDo not use web.fetch for this page URL; it may not have the user's browser session, cookies, or access.",
    ...
    ru: {
      ...
      summarizePagePrompt: "Суммируй эту страницу. Сфокусируйся на главных тезисах и полезных деталях.\n\nИспользуй Safari extension browser tool browser.dom_snapshot, чтобы прочитать содержимое активной страницы.\nНе используй web.fetch для URL этой страницы: у него может не быть браузерной сессии пользователя, cookies или доступа.",
    ...
    zh: {
      ...
      summarizePagePrompt: "总结此页面。请关注主要观点和可执行的细节。\n\n使用 Safari 扩展浏览器工具 browser.dom_snapshot 读取当前页面内容。\n不要对这个页面 URL 使用 web.fetch；它可能没有用户的浏览器会话、cookies 或访问权限。",
