...
func agentChatModeRuntimeInstructionsMatchModeSemantics() {
    ...
    #expect(build.contains("complete current checklist snapshot"))
    #expect(build.contains("Keep item `id` values stable"))
    #expect(build.contains("Do not duplicate the checklist as ordinary assistant Markdown"))
    #expect(build.contains("agents.delegate_task"))
    #expect(build.contains("at most 3"))
    #expect(build.contains("red-green-refactor"))
    #expect(build.contains("Build and Debug turns must call `session.complete`"))
    #expect(build.contains("verificationEvidenceIds"))
    #expect(build.contains("status=completed"))
    ...
    #expect(debug.contains("Build and Debug turns must call `session.complete`"))
    #expect(debug.contains("verificationEvidenceIds"))
    #expect(debug.contains("status=completed"))
