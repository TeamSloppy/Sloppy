...
func channelModelStoreGetSetAndPersist() async throws {
    ...
    await store.set(channelId: "general", model: "openai-api:gpt-5.4-mini")
    #expect(await store.get(channelId: "general") == "openai-api:gpt-5.4-mini")

    // Reload from disk
    let reloaded = ChannelModelStore(workspaceRootURL: dir)
    #expect(await reloaded.get(channelId: "general") == "openai-api:gpt-5.4-mini")
}
...
func channelModelStoreRemove() async throws {
    ...
    let store = ChannelModelStore(workspaceRootURL: dir)
    await store.set(channelId: "support", model: "openai-api:gpt-5.4-mini")
...
@Test
func channelChatModeStoreDefaultsToAuto() async throws {
    let dir = FileManager.default.temporaryDirectory
    ...
    #expect(await store.get(channelId: "general") == .auto)
}
...
func modeCommandNoLongerChangesChannelChatMode() async throws {
    ...
    #expect(await service.channelChatModeStore.get(channelId: channelId) == .auto)
}
