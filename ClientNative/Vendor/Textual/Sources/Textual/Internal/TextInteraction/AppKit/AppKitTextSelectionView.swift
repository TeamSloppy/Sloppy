  ...
  struct AppKitTextSelectionView: View {
    ...
    private func updateSelectionRects() {
      let updatedSelectionRects: [TextSelectionRect]
      if let textSelectionModel,
        let selectedRange = textSelectionModel.selectedRange
      {
        updatedSelectionRects = textSelectionModel.selectionRects(
          for: selectedRange,
          layout: layout
        )
      } else {
        updatedSelectionRects = []
      }

      guard updatedSelectionRects != selectionRects else { return }
      selectionRects = updatedSelectionRects
    }
  }
#endif
