...
struct AuthenticationScreenSourceTests {
    ...
    func logoAndPasswordAutoFillSemantics() throws {
        ...
        #expect(source.contains("authenticationField(\"Name\", text: $name)"))
        #expect(source.contains(".textContentType(.name)"))
        #expect(source.contains("secureAuthenticationField(\"Invite token\", text: $inviteToken)"))
        #expect(source.contains("apiClient.bootstrapIdentityAdmin("))
        #expect(source.contains("apiClient.registerIdentityUser("))
