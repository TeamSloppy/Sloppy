...
struct SloppyShellRenderingTests {
    ...
    @Test("main split layout uses navigation split view")
    func mainSplitLayoutUsesNavigationSplitView() throws {
        ...
        #expect(mainView.contains("NavigationSplitView"))
    ...
    @Test("glass shell composes blur and gradient overlay")
    func glassShellComposesBlurAndGradientOverlay() throws {
        let shell = try source("Sources", "SloppyClient", "SloppyGlassShell.swift")

        #expect(shell.contains(".ultraThinMaterial"))
