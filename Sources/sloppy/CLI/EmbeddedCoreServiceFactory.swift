...
struct EmbeddedCoreServiceFactory {
    static func make(configPath: String?, loggerLabel: String) async throws -> CoreService {
        ...
        var config = try loadServerConfigRecovering(
            from: explicitConfigPath, currentDirectory: homeDirectory)
        ...
        if explicitConfigPath == nil {
            ...
            if CoreConfigFileStore.hasConfigOrBackup(at: workspaceConfigPath) {
                ...
                config = try loadServerConfigRecovering(
                    from: workspaceConfigPath, currentDirectory: homeDirectory)
            }
        }

        let workspaceRoot = try prepareServerWorkspace(
            config: &config, currentDirectory: homeDirectory)
        ...
        if let error = CorePersistenceFactory.prepareSQLiteDatabaseIfNeeded(config: config) {
            logger.warning(
                "SQLite initialization failed at \(config.sqlitePath): \(error); runtime will use fallback persistence if needed"
            )
        }

        return CoreService(
            config: config, configPath: resolvedConfigPath, currentDirectory: homeDirectory)
    }
}
